...
def minify_js(content):
    ...
    # Sauvegarder les chaînes simple quotes et double quotes
    content = re.sub(r"'(?:[^'\\]|\\.)*'", save_string, content)
    content = re.sub(r'"(?:[^"\\\\]|\\.)*"', save_string, content)
    # Sauvegarder les template literals
    content = re.sub(r'`(?:[^`\\]|\\.)*`', save_string, content)
    ...
    # Supprimer les espaces autour des opérateurs (mais pas dans les chaînes)
    content = re.sub(r'\s*([{};,:])\s*', r'\1', content)
    content = re.sub(r'\s*([=<>!+\-*/])\s*', r'\1', content)
    # Supprimer les sauts de ligne inutiles
    content = content.replace('\n', '')

    # Restaurer les chaînes de caractères
    for i, string in enumerate(strings):
        content = content.replace(f'___STRING_{i}___', string)
